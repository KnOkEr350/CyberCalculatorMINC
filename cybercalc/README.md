...
│   ├── web/static/          # фронтенд: чистый HTML/CSS/JS без фреймворков
...
Backend поднимется на `http://localhost:8080` (порт настраивается через `HTTP_PORT`).
...
| Расчёт стоимости активностей по Приказу | `internal/calculators/*` — по одному файлу на категорию |
...
| Логи изменений в интерфейсе, хранение 2 месяца | `GET /api/admin/logs`, `internal/retention` (job раз в час) |
...
| Всё в compose, Dockerfile multi-stage | `docker-compose.yml`, `backend/Dockerfile` (builder → distroless) |

...
async function renderReportsScreen(root) {
  ...
  root.innerHTML = `<section class="page-heading screen-heading"><div><span class="eyebrow">Экран ${screen.number}</span><h1>${escapeHTML(screen.title)}</h1><p>${escapeHTML(screen.subtitle)}</p></div><span class="year-badge">${state.year}</span></section>
    ...
    <div class="report-format-grid">${reportLink(`/api/reports/export?${categoryQuery}`, "Excel по выбранному срезу", "Категория, партнёр, год и план/факт.", approved)}${reportLink(`/api/reports/export?${new URLSearchParams({ ...Object.fromEntries(base), report_type: "plan_fact" })}`, "План–факт–дельта", "Абсолютная и процентная дельта по партнёрам и видам.", approved)}${reportLink(`/api/reports/export?${new URLSearchParams({ ...Object.fromEntries(base), report_type: "annex1" })}`, "Приложение № 1", "Детализированный перечень мероприятий.", approved)}${reportLink(`/api/reports/export?${new URLSearchParams({ ...Object.fromEntries(base), report_type: "annex2" })}`, "Приложение № 2", "Реестр стажировок, часов и трудовых договоров.", approved)}${reportLink(`/api/reports/export?${new URLSearchParams({ ...Object.fromEntries(base), report_type: "annex2", mode: "mentors" })}`, "Отчёт по наставникам", "Часы сопровождения и закреплённые стажёры по каждому наставнику.", approved)}${reportLink(`/api/reports/export?${new URLSearchParams({ ...Object.fromEntries(base), report_type: "annex3" })}`, "Приложение № 3", "DOCX-справка при отсутствии мероприятий.", !!agreement)}${reportLink(`/api/reports/export?${new URLSearchParams({ ...Object.fromEntries(base), report_type: "annex5" })}`, "Приложение № 5", "Сводный отчёт и выполнение норматива 3%.", approved)}${reportLink(`/api/reports/export?${new URLSearchParams({ report_type: "annex4", report_year: state.year, ...(state.partnerID ? { partner_id: state.partnerID } : {}) })}`, "Приложение № 4", hasMaySnapshot ? "13 граф формы приказа № 270; факт читается из неизменяемого снимка на 1 мая." : "Сначала сформируйте снимок на 1 мая в настройках.", hasMaySnapshot)}${reportLink(`/api/reports/export?${new URLSearchParams({ ...Object.fromEntries(base), report_type: "agreement2" })}`, "Типовое соглашение № 2", "Автозаполнение реквизитов сторон в DOCX.", !!agreement)}${reportLink(`/api/reports/export?${new URLSearchParams({ ...Object.fromEntries(base), report_type: "agreement3" })}`, "Типовое соглашение № 3", "Автозаполнение реквизитов сторон в DOCX.", !!agreement)}${reportLink("", "Формы АНО АЦ", "Комплект ТОП-ИТ/ТОП-ИИ на шести листах требует серверного шаблона.", false)}</div>

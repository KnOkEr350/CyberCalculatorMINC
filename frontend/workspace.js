...
async function renderPartnerEntries(root) {
  ...
  </div>${state.me.role === "admin" || state.me.role === "moderator" || state.me.entity_type === "edu_institution" ? '<button class="btn secondary" id="open-directory">Справочник и соглашения</button>' : ""}<p class="context-status">${selectedAgreement ? `${escapeHTML(AGREEMENT_KIND_LABELS[selectedAgreement.agreement_kind] || selectedAgreement.agreement_kind)} · ${writable ? "Доступно редактирование" : "Только просмотр: проверьте статус и срок соглашения"}` : "Выберите партнёра и соглашение"}</p></div>

...
test("dashboard prioritizes operational indicators and moves analytics below", () => {
  ...
  for (const [state, color] of [["green", "#15966a"], ["yellow", "#e5b000"], ["red", "#d94b5f"]]) {
    assert.match(theme, new RegExp(`dashboard-risk-grid \\.risk-buckets > \\.${state} \\{[^}]*background: ${color}`, "s"));
  }
  assert.match(app, /class="\$\{stateName\} readiness-bucket"/);
  assert.doesNotMatch(app, /readiness-bucket-icon|readiness-bucket-meter|linear-gradient/);
});
...
test("activity screens use one-line plan/fact switch without target controls", () => {
  ...
  assert.match(workspace, /На этапе MVP выгрузка доступна в любом статусе/);
  assert.doesNotMatch(workspace, /Экспорт откроется после утверждения отчёта/);
});

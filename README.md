...
Актуальный сценарий работы от учебного заведения, роли, справочник, импорт Excel, вложения, ограничения и локальные проверки описаны в [WORKSPACE_CHANGES.md](WORKSPACE_CHANGES.md). Для этих разделов он уточняет историческое описание ниже и DECISIONS.md.
...
Интерфейс откроется на `http://localhost:8080`, а с другого устройства — на
`http://<IP-сервера>:8080`. По умолчанию nginx слушает `0.0.0.0`; адрес и порт
настраиваются через `HTTP_BIND` и `HTTP_PORT`. Внешний трафик принимает nginx:
запросы `/api/` балансируются между репликами backend, остальные запросы
направляются во frontend. Число реплик backend задаётся через
`BACKEND_REPLICAS` и по умолчанию равно двум.

Если сервер недоступен, проверьте состояние и последние сообщения контейнеров:

```bash
docker compose ps
docker compose logs --tail=100 nginx backend migrate db
curl -I http://127.0.0.1:8080
```

Пересобрать и запустить приложение можно командой:

```bash
docker compose up -d --build --remove-orphans
```

Если локальный `curl` работает, а подключение с другого устройства — нет,
проверьте, что TCP-порт 8080 разрешён в firewall сервера или облачной сети.
...
| Массовая загрузка ИТ-компаний | `GET /api/it-companies/template`, `POST /api/it-companies/import`, `scripts/sync-it-companies.sh` |
| Направления вузов по приказу № 27 | Точные коды в справочнике, `scripts/enrich-directory-programs.sh`; [инструкция](docs/DIRECTORY_AND_AGREEMENTS.md) |

...
Версионированный справочник ОКЗ, CSV-импорт и поиск описаны в [docs/OKZ_DIRECTORY.md](docs/OKZ_DIRECTORY.md).
...
├── migrations/              # SQL-миграции (one-shot сервис migrate)
├── nginx/
...
`BACKEND_REPLICAS` и по умолчанию равно двум. Очистку, синхронизацию и обогащение
справочников выполняет отдельный `worker`, поэтому масштабирование API не дублирует
фоновые задачи.
...
```bash
docker compose ps
docker compose logs --tail=100 nginx backend worker migrate db
...
Пробы: `/api/live` проверяет только HTTP-процесс, `/api/ready` — готовность API и PostgreSQL.
Прежний `/api/health` сохранён как alias `/api/ready`.
